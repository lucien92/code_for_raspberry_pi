...
class BoundBox:
    ...
    def get_score(self):
        ...
        print("conf en la classe", self.classes[self.get_label()])
        print("le produit conf en class et pred vaut:", self.score)
        return self.score
...
def decode_netout(netout, anchors, nb_class, obj_threshold=0.5, nms_threshold=0.3): #on transforme ce qui sort du réseau en bounding boxes
    ...
    print("grid_h, grid_w, nb_box", grid_h, grid_w, nb_box)
    ...
    for row in range(grid_h):
        for col in range(grid_w):
            for b in range(nb_box):
                ...
                #print("classes", classes) #affiche les proba d'identification de chaque classe
                confidence = netout[row, col, b, 4]
                
                if confidence >= obj_threshold:
                    #print("confidence >= obj_threshold", confidence) #confiance en la prédiction
                    ...
                    #print("confidence louche", confidence)
                    box = BoundBox(x - w / 2, y - h / 2, x + w / 2, y + h / 2, confidence, classes)
                    #print("look my boxes",box)
                    boxes.append(box)
    #print("before suppress non-maximal",boxes)
